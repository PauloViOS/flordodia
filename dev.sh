...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=flordodia.example.com
...
workon flordodia  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development flordodia npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop flordodia
    docker rm flordodia
    docker run --name flordodia -d --env-file /home/ubuntu/flordodia.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/flordodia:/dkdata \
        flordodia start_web.sh
}
...
echo_green "Welcome to flordodia's dev env"
